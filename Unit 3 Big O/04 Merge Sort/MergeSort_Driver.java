...
class MergeSort
{
   ...
   */
                                   ...
                                   int low, int middle, int high)
   {
      ...
      // Interleave items between low and high into the copyBuffer's low and high always taking the lower of the values indexed by i1 and i2
      for(int counter = low; counter <= high; counter++){
         if(i1 > middle){
            copyBuffer[counter] = array[i2];
            i2++;
         }
         else if(i2 > high){
            copyBuffer[counter] = array[i1];
            i1++;
         }
         else if(!(array[i1] > array[i2] || array[i1] == array[i2])){
            copyBuffer[counter] = array[i1];
            i1++;
         }
         else{
      ...
      // then copy the just-sorted items between low and high from the copyBuffer back to the array.
...
class SelectionLowHigh
{
   ...
   {
      while(high>low){
         swap(array, findMax(array, low, high+1), high);
         high--;
      }
   }
